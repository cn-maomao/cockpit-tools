...
- **Grok 全自动工具链**：新增原生 Grok 工具页，可自动启动浏览器注册账号、捕获账号结果、导入内置 Grok2API，并生成可直接使用的 OpenAI 兼容 Base URL 与 API Key。
- **内置 Grok sidecar**：集成 `grok-register` 与 `grok2api`，由 Cockpit 统一管理配置、密钥、进程、健康状态、日志及退出清理。
